...
export default function PublicTemplePage() {
    ...
    async function fetchTabData() {
      try {
        if (activeTab === "overview") {
          const res = await api.get(`/public/${slug}/summary`, {
            params: { year: year || undefined },
          });
          setSummary(res.data);
        } else if (activeTab === "videos") {
          const res = await api.get(`/public/${slug}/videos`, {
            params: { year: year || undefined },
          });
          setVideos(res.data);
        } else if (activeTab === "events") {
          const res = await api.get(`/public/${slug}/daily-events`, {
            params: { year: year || undefined },
          });
          setEvents(res.data);
        }
 ...
 console.log("Full URL (should just be the Cloudinary URL itself):", org.backgroundUrl);
  return (
  ...
  ? `url(${org.backgroundUrl})`   // ✅ correct — org.backgroundUrl is already a full URL
              ...
              <FadeInOnScroll>
                <div className="bg-white/80 backdrop-blur-xl border border-white/50 p-5 rounded-xl shadow-lg text-center transition-transform duration-200 hover:scale-105 hover:shadow-xl active:scale-95">
                  ...
                  <p className="text-2xl font-bold text-green-700">
                    ₹{summary.totalChanda.toLocaleString("en-IN")}
                  </p>
                </div>
              </FadeInOnScroll>
              ...
              <FadeInOnScroll>
                <div className="bg-white/80 backdrop-blur-xl border border-white/50 p-5 rounded-xl shadow-lg text-center transition-transform duration-200 hover:scale-105 hover:shadow-xl active:scale-95">
                  ...
                  <p className="text-2xl font-bold text-purple-700">
                    ₹{(summary.totalEventBudget || 0).toLocaleString("en-IN")}
                  </p>
                </div>
              </FadeInOnScroll>
            ...
            <FadeInOnScroll>
              <div className="bg-white/80 backdrop-blur-xl border border-white/50 rounded-xl shadow-lg p-8 text-center">
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Detailed {activeTab === "chanda" ? "chanda" : "expense"}{" "}
                  records are only visible to temple administrators.
                </p>
              </div>
            </FadeInOnScroll>
          )}
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="font-medium text-gray-800">
                            {ev.eventType.value === "Other"
                              ? ev.eventType.customValue
                              : ev.eventType.value}
                          </p>
                          {ev.description && (
                            <p className="text-sm text-gray-500">
